...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!openaiKey && !anthropicKey) {
      return json({ error: "No AI API keys configured (OPENAI_API_KEY or ANTHROPIC_API_KEY required)." }, 500);
    }
    ...
    const logRows: {
      ...
      provider?: string; fallback_used?: boolean; model?: string;
    }[] = [];
      ...
      try {
        ...
        if (openaiKey) {
          try {
            ...
            continue; // Success, move to next template
          } catch (err) {
            primaryError = err instanceof Error ? err : new Error(String(err));
            console.warn(`[OpenAI] Failed for ${tpl.section_key}:`, primaryError.message);
            
            // Check if we should fallback
            const shouldFallback = primaryError.message.includes("429") || 
                                   primaryError.message.includes("503") || 
                                   primaryError.message.includes("504") ||
                                   primaryError.message.includes("timeout");
            
            if (!shouldFallback || !anthropic) {
              throw primaryError; // Don't fallback, throw error
            }
            
            console.log(`[Anthropic] Attempting fallback for ${tpl.section_key}...`);
          }
        }

        // Fallback to Anthropic (or use as primary if no OpenAI key)
        if (anthropic) {
          const anthropicModel = modelMapping[tpl.model || "gpt-4o"] || "claude-3-5-sonnet-20241022";
          ...
          throw primaryError || new Error("No AI provider available");
        }
      } catch (err) {
