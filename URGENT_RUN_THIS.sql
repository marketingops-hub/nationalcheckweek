-- ═══════════════════════════════════════════════════════════════════
-- URGENT FIX: Create ambassador_categories table
-- Run this in: Supabase Dashboard → SQL Editor → New Query
-- ═══════════════════════════════════════════════════════════════════

-- ── 1. Create set_updated_at function if it doesn't exist ──────────
...
-- ── 2. Create ambassador_categories table ──────────────────────────
...
drop policy if exists "Public read ambassador_categories" on ambassador_categories;
create policy "Public read ambassador_categories" on ambassador_categories for select using (true);

drop policy if exists "Auth write ambassador_categories" on ambassador_categories;
create policy "Auth write ambassador_categories" on ambassador_categories for all using (auth.role() = 'authenticated');

-- ── 4. Create trigger ────────────────────────────────────────────────
drop trigger if exists ambassador_categories_updated_at on ambassador_categories;
...
-- ── 6. Add category_id to Ambassador table ───────────────────────────
...
-- ── 7. Verify ─────────────────────────────────────────────────────────
select 'ambassador_categories table created successfully!' as status;
select count(*) as category_count from ambassador_categories;
