...
export default function PartnersClient() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      const res = await adminFetch('/api/admin/partners?all=true');
      const d = await res.json();
  ...
  const handleCreate = async (form: FormData) => {
    ...
    try {
      const res = await adminFetch('/api/admin/partners', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
      const d = await res.json();
  ...
  const handleUpdate = async (id: string, patch: Partial<Partner> | FormData) => {
    setUpdatingId(id);
    try {
      const res = await adminFetch(`/api/admin/partners/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(patch) });
      const d = await res.json();
