...
interface StateIssue { name: string; badge: string; stat: string; desc: string; slug?: string; }
interface GlobalIssue { slug: string; title: string; }
...
function StateIssueCard({ issue, idx, onChange, onRemove, globalIssues }: {
  ...
  return (
    <div className="rounded-xl p-4" style={{ background: "var(--admin-bg-elevated)", border: "1px solid var(--admin-border)", borderLeft: `3px solid ${leftColor}` }}>
      ...
      <Field label="Links to Issue Page">
        <select className={I} style={IS} value={issue.slug ?? ""} onChange={e => onChange(idx, "slug", e.target.value)}>
...
export default function StateEditForm({ state }: { state: State | null }) {
  ...
  return (
    <div className="max-w-4xl">
      ...
      {/* Tab bar - using new admin-editor-tabs classes ✅ */}
      <div className="admin-editor-tabs">
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`admin-editor-tab ${tab === t.id ? 'active' : ''}`}>
            {t.label}
            {t.count !== null && (
              <span className="text-xs px-1.5 py-0.5 rounded-full font-bold ml-2"
                style={tab === t.id
        ...
        <div className="admin-card">
          ...
          {issues.length === 0 ? (
            ...
            <div className="space-y-3">
              {issues.map((iss, idx) => <StateIssueCard key={idx} issue={iss} idx={idx} onChange={updateIssue} onRemove={removeIssue} globalIssues={globalIssues} />)}
            </div>
          )}
        </div>
      )}
