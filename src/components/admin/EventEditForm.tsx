...
import SpeakerCard from "@/components/admin/events/SpeakerCard";
...
const EMPTY: EventForm = {
  ...
  register_url: "", recording_url: "", hubspot_form_id: "", hubspot_portal_id: "", status: "upcoming",
    ...
    <div>
      ...
      {tab === "details" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div>
              <label className="swa-form-label">HubSpot Form ID</label>
              <input className={I} value={form.hubspot_form_id} onChange={(e) => set("hubspot_form_id", e.target.value)} placeholder="abc123-def456-..." />
            </div>
            <div>
              <label className="swa-form-label">HubSpot Portal ID</label>
              <input className={I} value={form.hubspot_portal_id} onChange={(e) => set("hubspot_portal_id", e.target.value)} placeholder="12345678" />
            </div>
          </div>
          <p style={{ fontSize: "0.82rem", color: "#6B7280", marginTop: -8, marginBottom: 16 }}>
            If both IDs are provided, a HubSpot registration form will be embedded on the event page (same style as /register page).
          </p>
