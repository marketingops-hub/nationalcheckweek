    ...
    <>
      ...
      <div className="area-hero">
        ...
        {/^\s*</.test(area.overview ?? "") ? (
          <div className="area-hero-sub" dangerouslySetInnerHTML={{ __html: sanitizeHtml(area.overview) }} />
        ) : (
      ...
      <div className="area-body">
            ...
            {areaIssues.map((issue, i) => {
              ...
              const card = (
                <div className={`area-issue-card ${issue.severity}${issueSlug ? " area-issue-card--linked" : ""}`}>
                  ...
                  {issueSlug && (
                    <div className="area-issue-cta">Read deep dive →</div>
                  )}
                </div>
              );
              return issueSlug ? (
                <Link key={i} href={`/issues/${issueSlug}`}>{card}</Link>
              ) : card;
            })}
          </div>
        </div>
        ...
        <div className="area-section">
          ...
          <div className="area-prevention">
            ...
            {/^\s*</.test(area.prevention ?? "") ? (
              <div dangerouslySetInnerHTML={{ __html: sanitizeHtml(area.prevention) }} />
            ) : (
              <p>{area.prevention}</p>
            )}
        ...
        {/* Sources - Dynamic from database */}
        <SourcesList entityType="area" entitySlug={slug} />
